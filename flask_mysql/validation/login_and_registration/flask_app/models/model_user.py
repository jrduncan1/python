...
DATABASE_SCHEMA = 'login_registration_schema'
...
class User:
    ...
    @classmethod
    def retrieve_with_email(cls, data):
    ...
    def registration_validate(check_user):
        ...
        if not EMAIL_REGEX.match(check_user['email']): 
            ...
            user = User.retrieve_with_email({'email': check_user['email']})
            if user:
