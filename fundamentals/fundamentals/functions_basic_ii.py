...
# Return a new list that counts down by one, from the number (as the
# 0th element) down to 0 (as the last element)
# Example: countdown(5) shoud return [5,4,3,2,1,0]

def count_down(number):
    countdownList = []
    for idx in range(number,-1,-1):
        countdownList.append(idx)
...
def first_plus_length(list):
    sum = list[0] + len(list)
    return sum
print(first_plus_length([100,20,3]))

# 4. Values Greater than Second - Write a function that accepts a list and creates a new list
# containing only the values from the original list that are greater than its 2nd value (index 1).
...
def greater_than_second(list):
    ...
    print(len(newList))
    return newList
print(greater_than_second([1,5,10,2,11,25]))
print(greater_than_second([100]))
